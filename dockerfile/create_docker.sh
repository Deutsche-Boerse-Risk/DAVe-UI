...
docker tag ${DOCKER_ORG_NAME}/${DOCKER_IMAGE_NAME}:${GIT_SHA1} $DOCKER_HUB/${DOCKER_ORG_NAME}/${DOCKER_IMAGE_NAME}:${GIT_SHA1}
docker tag ${DOCKER_ORG_NAME}/${DOCKER_IMAGE_NAME}:${GIT_SHA1} $DOCKER_HUB/${DOCKER_ORG_NAME}/${DOCKER_IMAGE_NAME}:${GIT_BRANCH}

# Push the image
docker login -e="$DOCKER_EMAIL" -u="$DOCKER_USERNAME" -p="$DOCKER_PASSWORD" $DOCKER_HUB
docker push $DOCKER_HUB/${DOCKER_ORG_NAME}/${DOCKER_IMAGE_NAME}:${GIT_SHA1}
docker push $DOCKER_HUB/${DOCKER_ORG_NAME}/${DOCKER_IMAGE_NAME}:${GIT_BRANCH}
